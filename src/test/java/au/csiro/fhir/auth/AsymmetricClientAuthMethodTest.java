...
import static au.csiro.fhir.auth.TestUtils.getResourceAsString;
...
/**
 * Tests for {@link AsymmetricClientAuthMethod}.
 *
 * @author Piotr Szul
 */
class AsymmetricClientAuthMethodTest {
  ...
  void createsCorrectAssertionAndHeadersForRS384Key() throws JOSEException, ParseException {
    ...
    final AsymmetricClientAuthMethod credentials =
        AsymmetricClientAuthMethod.builder()
    ...
    assertEquals(
        ...
        authAssertions.stream()
            .filter(p -> p.getName().equals("client_assertion_type"))
            .findFirst()
            .orElseThrow()
            .getValue());

...
public class SMARTTokenCredentialFactory implements TokenCredentialFactory {
  ...
  @Value
  static class ProviderTokenCredentials implements TokenCredentials {

    @Nonnull AuthTokenProvider authTokenProvider;

    @Nonnull ClientAuthMethod clientAuthMethod;
  ...
  @Override
  @Nonnull
  public Optional<TokenCredentials> createCredentials(
      @Nonnull final URI fhirEndpoint, @Nonnull final AuthConfig authConfig) {
    return createAuthMethod(fhirEndpoint, authConfig)
        .map(authMethod -> new ProviderTokenCredentials(authTokenProvider, authMethod));
  }
  ...
  @Nonnull
  Optional<? extends ClientAuthMethod> createAuthMethod(
      @Nonnull final URI fhirEndpoint, @Nonnull final AuthConfig authConfig) {
  ...
  @Nonnull
  private ClientAuthMethod createSMARTAuthMethod(
      @Nonnull final URI fhirEndpoint, @Nonnull final AuthConfig authConfig) {
    try {
      log.debug("Retrieving SMART configuration for fhirEndpoint: {}", fhirEndpoint);
      final SMARTDiscoveryResponse discoveryResponse =
          SMARTDiscoveryResponse.get(fhirEndpoint, httpClient);
      ...
      log.error("Failed to retrieve SMART configuration for fhirEndpoint: {}", fhirEndpoint, ex);
  ...
  public static SMARTTokenCredentialFactory create(@Nonnull final AuthConfig configuration) {
    ...
    return new SMARTTokenCredentialFactory(
        httpClient, new AuthTokenProvider(httpClient, configuration.getTokenExpiryTolerance()));
  }
}

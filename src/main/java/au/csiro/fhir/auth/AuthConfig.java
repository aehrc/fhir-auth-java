...
import au.csiro.fhir.auth.ValidationUtils.ViolationAccumulator;
...
/**
 * Configuration relating to authentication of requests to a FHIR server.
 *
 * @author John Grimes
 * @author Piotr Szul
 */
...
public class AuthConfig implements Serializable {
  ...
  /** Enables authentication of requests to the FHIR server. */
  @Builder.Default private boolean enabled = false;

  /** Use SMART configuration to discover token endpoint. */
  @Builder.Default private boolean useSMART = true;

  /**
   * An OAuth2 token endpoint for use with the client credentials grant. Only applicable if {@link
   * #useSMART} is false.
   */
  ...
  /**
   * Send the basic authentication credentials in the form body instead of the Authorization header.
   */
  @Builder.Default private boolean useFormForBasicAuth = false;
  ...
  /** Annotation for validating AuthConfig. */
  ...
  public static class AuthConfigValidator
      implements ConstraintValidator<ValidAuthConfig, AuthConfig> {

    @Override
    public boolean isValid(final AuthConfig value, final ConstraintValidatorContext context) {
      if (value.isEnabled()) {
        return ViolationAccumulator.withNoDefault(context)
            .checkThat(
                value.isUseSMART() || value.getTokenEndpoint() != null,
                "must be supplied if SMART configuration is not used and auth is enabled",
                "tokenEndpoint")
            .checkThat(
                value.getClientId() != null, "must be supplied if auth is enabled", "clientId")
            .checkThat(
                value.getClientSecret() != null || value.getPrivateKeyJWK() != null,

...
class JWTUtils {
  ...
  public static Algorithm getAsymmSigningAlgorithm(@Nonnull final JWK privateKeyJWT) {
    ...
    try {
      ...
      final Method algoFactoryMethod =
          Arrays.stream(Algorithm.class.getDeclaredMethods())
              ...
              .findFirst()
              .orElseThrow(
                  () -> new IllegalArgumentException("Unsupported algorithm: " + jwtAlgoName));
  ...
  private static String toAuto0AlgoName(@Nonnull final String jwkAlgorithm) {
    return jwkAlgorithm.replace("RS", "RSA").replace("ES", "ECDSA").replace("HS", "HMAC");
  }
}

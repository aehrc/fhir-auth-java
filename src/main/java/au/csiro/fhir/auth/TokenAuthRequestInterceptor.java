...
/**
 * {@link HttpRequestInterceptor} that performs preemptive bearer token authentication.
 *
 * <p>The {@link CredentialsProvider} configured with the {@link HttpClientContext} should provide
...
public class TokenAuthRequestInterceptor implements HttpRequestInterceptor {
  ...
  public void process(@Nonnull final HttpRequest request, @Nonnull final HttpContext context) {
    ...
    if (credentialsProvider != null && targetHost != null) {
      final Optional<Credentials> maybeCredentials =
          Optional.ofNullable(credentialsProvider.getCredentials(new AuthScope(targetHost)));
      ...
      maybeCredentials
          ...
          .ifPresent(
              authToken -> {
                log.debug(
                    "Adding access token: {} to request: {}", authToken, request.getRequestLine());
                request.addHeader(

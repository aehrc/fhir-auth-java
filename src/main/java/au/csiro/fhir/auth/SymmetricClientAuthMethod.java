...
public class SymmetricClientAuthMethod extends ClientAuthMethod {
  ...
  @Builder.Default boolean sendClientCredentialsInBody = false;

  @Nonnull
  String toAuthString() {
    return Base64.getEncoder()
        .encodeToString((clientId + ":" + clientSecret).getBytes(StandardCharsets.US_ASCII));
  }
  ...
  public List<Header> getAuthHeaders() {
    return sendClientCredentialsInBody
        ...
        : List.of(
            new BasicHeader(
                HttpHeaders.AUTHORIZATION, AuthConst.AUTH_BASIC + " " + toAuthString()));
  }
}

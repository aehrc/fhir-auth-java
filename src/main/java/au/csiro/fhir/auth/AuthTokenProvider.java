 ...
 *
 * <p>The tokens are cached in memory and are reused if they are still valid for at least the time
...
public class AuthTokenProvider {
   ...
   */
  public @Nonnull Token getToken(@Nonnull final ClientAuthMethod authMethod) {
    try {
  ...
  private AccessContext ensureAccessContext(
      @Nonnull final ClientAuthMethod credentials, final long tokenExpiryTolerance)
      throws IOException {
    synchronized (accessContexts) {
      ...
      if (accessContext == null
        ...
        accessContext = getNewAccessContext(credentials, tokenExpiryTolerance);
